 ...
 * @param    []models_pkg.Status6Enum        status     parameter: Required
 * @return	Returns the []*models_pkg.Pet response from the API call
 */
func (me *PET_IMPL) FindPetsByStatus (
            status []models_pkg.Status6Enum) ([]*models_pkg.Pet, error) {
        //the base uri for api requests
    _queryBuilder := configuration_pkg.GetBaseURI(configuration_pkg.ENUM_DEFAULT,me.config);
   ...
   _queryBuilder = _queryBuilder + "/pet/findByStatus"
    ...
    _queryBuilder, err = apihelper_pkg.AppendUrlWithQueryParameters(_queryBuilder, map[string]interface{} {
        "status" : models_pkg.Status6EnumArrayToValue(status),
    })
    if err != nil {
